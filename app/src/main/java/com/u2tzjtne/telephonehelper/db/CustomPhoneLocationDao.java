...
public interface CustomPhoneLocationDao {
    ...
    Completable insert(CustomPhoneLocation location);

    /**
     * 根据号码前缀查询归属地
     * @param phone 号码前缀（去掉后4位）
     */
    @Query("SELECT * FROM custom_phone_location WHERE phone = :phone LIMIT 1")
    Single<CustomPhoneLocation> findByPhone(String phone);
     ...
     * 根据号码删除
     */
    @Query("DELETE FROM custom_phone_location WHERE phone = :phone")
