...
class BadgeRuleBindingAdapter(
    private val onDeleteClick: (PhoneBadgeRuleBinding) -> Unit,
) : ListAdapter<PhoneBadgeRuleBinding, BadgeRuleBindingAdapter.ViewHolder>(DiffCallback()) {
    ...
    inner class ViewHolder(
        ...
        fun bind(item: PhoneBadgeRuleBinding) {
            binding.tvPhoneNumber.text = formatPhoneNumber(item.phoneNumber)
            binding.tvPlayType.text = item.ruleName
    ...
    class DiffCallback : DiffUtil.ItemCallback<PhoneBadgeRuleBinding>() {
        override fun areItemsTheSame(
            oldItem: PhoneBadgeRuleBinding,
            newItem: PhoneBadgeRuleBinding,
        ): Boolean {
