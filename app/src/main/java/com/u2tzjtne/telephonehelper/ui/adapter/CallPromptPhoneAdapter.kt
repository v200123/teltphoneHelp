...
class CallPromptPhoneAdapter(
    private val onDeleteClick: (CallPromptPhone) -> Unit
) : ListAdapter<CallPromptPhone, CallPromptPhoneAdapter.ViewHolder>(DiffCallback()) {
    ...
    class DiffCallback : DiffUtil.ItemCallback<CallPromptPhone>() {
        override fun areItemsTheSame(oldItem: CallPromptPhone, newItem: CallPromptPhone): Boolean {
            return oldItem.id == newItem.id
        }

        override fun areContentsTheSame(oldItem: CallPromptPhone, newItem: CallPromptPhone): Boolean {
            return oldItem.phoneNumber == newItem.phoneNumber &&
                oldItem.promptType == newItem.promptType &&
